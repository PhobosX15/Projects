...
import com.mygdx.game.bot.GA_bot;
    ...
    public Course(String fileName) {
        ...
        DragListener listener = new DragListener() {
            ...
            public boolean touchDown(InputEvent event, float x, float y, int pointer, final int button) {
                ...
                TextButton GAButton = new TextButton("Genetic Algorithm", skin);
                GAButton.addListener(new ClickListener(){
                    @Override
                    public void clicked(InputEvent event, float x ,float y){
                        SequenceAction sequenceAction = new SequenceAction();
                        Vector2d newposition = new Vector2d(ball.getX(), ball.getY());
                        Vector2d ballposition= new Vector2d(ball.getX(), ball.getY());
                        double distanceToHole= Math.sqrt(Math.pow((holeCoord.get_x()-ballCoord.get_x()),2)+ Math.pow((holeCoord.get_y()-ballCoord.get_y()),2));
                        System.out.println("Distance to Hole="+ distanceToHole);
                        while(distanceToHole> hole_tolerance)
                        {
                            GA_bot ga_bot = new GA_bot(ballposition, holeCoord, eulerSolver, formula,mass, gravitationalAcceleration,stepSize);
                            Vector2d shot = ga_bot.run_GA();
                            running= true;
                            velocity = new Vector2d( shot.get_x(), shot.get_y());
                            int i=0;
                            while(running){
                                i++;
                                newposition = throwBall(newposition);
                            ...
                            System.out.println("Ball after shot " + ball.getX() + " " + ball.getY());
                            distanceToHole = Math.sqrt(Math.pow((holeCoord.get_x()-ballCoord.get_x()),2)+ Math.pow((holeCoord.get_y()-ballCoord.get_y()),2));
                            ballposition = new Vector2d(ball.getX(), ball.getY());
                            dialog.hide();
